...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.infer_last = nn.Sequential(
            ...
            layer_init(nn.Linear(64 * 7 * 7, 512)),
            nn.ReLU(),
            layer_init(nn.Linear(512 , 6), std=0.01),
        )
        self.envs = envs
    ...
    def get_action_and_value(self, x, action=None):
        ...
        hidden_2 = self.infer_last(x_diff / 255.0)
        probs_oldaction = Categorical(logits=hidden_2)
        last_action = probs_oldaction.sample()
        sta_act_pair = torch.cat([hidden, last_action.unsqueeze(1)], 1)
