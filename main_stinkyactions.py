...
import numpy as np
from PPO_stinkyactions import Agent
...
def train_ppo(agent, device, run_name, total_timesteps, seed, num_steps, num_envs, minibatches, learning_rate, anneal_lr=True, gamma=0.99,
    ...
    actions_old = torch.zeros((num_steps + 1, num_envs)+ envs.single_action_space.shape).to(device)
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        for k in range(0, num_steps):
            ...
            with torch.no_grad():
                ...
                logprobs[k] = logprob
            # sample the action
            action = sample_actions(action, actions_old[k].long(), 0.75) # 75% probs to choose the present action
            if k < num_steps - 1:
            ...
            if "final_info" in infos:
                for info in infos["final_info"]:
                    if info and "episode" in info:
                        ...
                        idx = infos["_final_info"].tolist().index(True) # Find the finished agents
